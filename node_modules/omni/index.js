...
module.exports = function Omni (id, template) { // >>>>>>>>>>>>>>>>>>>>>>>>>>>>
  ...
  D(document.body,template.get('.omni'),utils.UID('omni')).draw(function (el) {
    omni = el
    live = true
    ...
    notify = D(el.querySelector('.messages'), template.get('.notification')).draw
    fil({txt:om.filters[om.filterIndex[0]][0]})
