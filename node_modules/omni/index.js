...
module.exports = function Actions (id, template) {
  ...
  // dom >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  var ctx = D(document.body, template.get('.ctx'))
  var om = D(document.body, template.get('.omni'), utils.UID('omni'))
  var drop = D(document.body, template.get('.dropzone'))
  var notify
  var disp
  var act
  ...
  window.addEventListener('click', function (e) {
    if (ctxLive === true) { 
      document.querySelector('.ctx').style.display = 'none'
      ctxLive = false
    }
  }, false)
  ...
  function contextMenu (e) { 
    ...
    drop({
      ...
      w : element.offsetWidth+'px',
      h : element.offsetHeight+'px'
    }, function (dz) {
    ...
    ctx({options:[{opt:'replace image...'}]}, function (el) {
      el.style.left = e.clientX - 10 +'px'
      el.style.top = e.clientY - 10 +'px'
    })
    if (document.querySelector('.ctx'))
      document.querySelector('.ctx').style.display = 'block'
    console.log(id, parent, tag, cl)
  }
