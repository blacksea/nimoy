...
module.exports = function Omni (id, template) { // >>>>>>>>>>>>>>>>>>>>>>>>>
  ...
  function keyMappings (e) {
    if (e.keyCode === 27) { e.preventDefault(); omniBox.toggle() }
    if (e.keyCode === 40 || e.keyCode === 38 && live) {
      e.preventDefault() 
      console.log(e.keyCode)
      var mode = (!e.shiftKey) ? 'filter' : 'action'
      var incdec = (e.keyCode===38) ? 'dec' : 'inc'
      omniBox.sel(mode, incdec) 
    }
    if (e.keyCode === 9 && live === true) { e.preventDefault(); toggleMode() }
    if (!e.shiftKey && e.keyCode === 13 && live) { 
      e.preventDefault(); omniBox.exec() 
    }
  } // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  var omniBox = { // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    input : function (e) {
      ...
      var res = utils.search(library.master, str) // use filter for input

      if (res && str !== '') {
        omniBox.value = res
        disp({txt : res.name})
        act({txt:omniBox.filters[omniBox.filterIndex[0]][omniBox.filterIndex[1]]})
      } else if (!res || str === '') {
        omniBox.value = null
        disp({txt : ''})
        act({txt : ''})
    ...
    exec : function () {
      var act = omniBox.filters[omniBox.filterIndex[0]][omniBox.filterIndex[1]]
      s.push({
        key : act,
        value : omniBox.value.name
      })
    },
    toggle : function () {
      if (live === true) {
    ...
    sel : function (mode, incdec) {
      if (mode === 'action') {
        var max = omniBox.filters[omniBox.filterIndex[0]].length-1
        var val = omniBox.filterIndex[1]
        omniBox.filterIndex[1] = utils.incdec(incdec, val, 1, max)
        act({txt:'> '+omniBox.filters[omniBox.filterIndex[0]][omniBox.filterIndex[1]]})
      } else if (mode === 'filter') {
        var val = omniBox.filterIndex[0]
        var max = omniBox.filters.length-1
        omniBox.filterIndex[0] = utils.incdec(incdec, val, 0, max)
        fil({txt:omniBox.filters[omniBox.filterIndex[0]][0]})
        act({txt:'> '+omniBox.filters[omniBox.filterIndex[0]][omniBox.filterIndex[1]]})
  ...
  om(function (el) {
    ...
    fil({txt:omniBox.filters[omniBox.filterIndex[0]][0]})
    el.querySelector('#search').addEventListener('keyup', omniBox.input, false)
