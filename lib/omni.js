...
module.exports = function Omni (opts) {
  ...
  var dx = {w : window.innerWidth, h : window.innerHeight } // crap data init
  ...
  if (localStorage['omniPos']) {
    dx.x = JSON.parse(localStorage['omniPos'])[0]
    dx.y = JSON.parse(localStorage['omniPos'])[1]
  }
  ...
  var omni = new D({ 
    ...
    events : [
      ...
      ['.omni','mousedown',function(e){dragPanel(e)}],
      [window,'mouseup',function(e){dragPanel()}], // not great
  ...
  if (!sessionStorage['edit'])
    login.draw({x:dx.x,y:dx.y,err:false})

  ps(dx.cvs) // hmph!

  function IO (d, enc, next) {  // EVENTS
    if (d.code === 1) { 
      next() 
