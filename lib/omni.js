...
module.exports = function Omni (opts) {
  ...
  var editor = new D({
    ...
    events : [
      ...
      ['.grifter .rm', 'click', function (e) {
        cancel(e); brico('-'+blob.cuid); editor.erase()
      }],
  ...
  document.addEventListener('drop', function (e) { // symbols ◼ ψ
    ...
    if (e.dataTransfer.files.length > 0) {
      upload(e, function (ndata) { }) // cmd = str // console.log(ndata) })
    } else cmd = e.dataTransfer.getData('cmd')
    if (cmd) brico(cmd+'/'+opts.id)
    else return false
  },false)
    ...
    if (e && e.keyCode === 13) { // try to grab data first!
      ...
      db.get('?#'+e.target.value, function (d) {
        if (d instanceof Error) {
          ps(e.target.value)
          e.target.blur()
          brico('+#'+window.location.pathname.replace('/','')+'/'+opts.id)
        } else {
          // brico('!#'+window.location.pathname.replace('/','')+'/'+opts.id)
        }
      })
  ...
  function editOUT (e) { // some intelligent feedback / filtering
    ...
    if (e.keyCode === 13) {
      if (key==='+copy') {
        ...
        blob.data[key] = val
        brico({key : '+$'+blob.cuid+'/'+opts.id, value:blob.data})
        editor.erase()
      }
    }
  }
