...
module.exports = function Omni (opts) {
  ...
  function pjsInit () {
    ...
    function drawSkeleton () {
      var scaleX = pjs.view.bounds.width/pageWidth()
      var scaleY = (pjs.view.bounds.height-15)/pageHeight()//15=tabHeight

      if (skeleton) skeleton.remove()

      skeleton = new pjs.Layer({
        position : [0.0]
      })
      ...
      skeleton.addChild(bg)

      skeleton.visible = true
      _.each(frame, function (v,k) {
        var el = document.getElementById(k).firstChild
        // var dim = [el.offsetWidth,el.offsetHeight]
        var pos = [
          (el.offsetLeft+(el.offsetWidth/2))*scaleX,
          (el.offsetTop+(el.offsetHeight/2))*scaleY
        ] 

        console.log(pos)

        var m = new pjs.Shape.Circle({
          center : pos,
          radius : 10,
          fillColor : 'red',
          strokeWidth : 0
    ...
    function scrollSkeleton (e) {

    }

    window.onscroll = scrollSkeleton
