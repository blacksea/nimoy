...
module.exports = function Omni (opts) {
  ...
  function uiInit () {
    ...
    function tab (label,x,y,w,h) {
      var bg = new pjs.Path([ [0,0], [w,0], [w-h,h], [0+h,h] ])
      var l = new pjs.PointText({
        point : [0,0],
        content : label,
        fontFamily : 'bmono',
        fontSize : 10,
      ...
      window.l = l
      var g = new pjs.Group([bg,l])
      g.children[1].position = g.children[0].position
      g.position = [x+(w/2),y+(h/2)]
      return g
    }
    Ltab = tab('Pallette',20,(pjs.view.bounds.height-24),90,18)
    Rtab = tab('Skeleton',110-18,(pjs.view.bounds.height-24),90,18)
    ...
    function tabSelect (e) {
      var t = e.parent
      t.children[0].fillColor = selectColor
    }
    ...
    var bbg = new pjs.Shape.Rectangle({ // CANVAS BG
      point : [35, 45],
      size : [130, window.innerHeight*(130/window.innerWidth)]
    })
    bbg.sendToBack()

    var bolt = new pjs.Path([ // GLUED TO BOTTOM
    ...
    bolt.fillColor = '#E5E5DC'

    var lin = new pjs.Path([[0,0],[0,pjs.view.bounds.height-24]])
    lin.strokeColor = 'rgba(0,0,0,0.3)'

    var panel = new pjs.Shape.Rectangle({
      point : [0,0],
    ...
    Ltab.onClick = function (e) {console.log(e);tablle(Ltab)}
    Rtab.onClick = function (e) {tablle(Rtab)}

    var dataCap = new pjs.Path.Circle({
      center:[13,sH-12],
      radius:6,
      fillColor:'blue'
    })
    dataCap.onClick = toggleTray

    function toggleTray (e) {
      console.log(e)
    }

    var sets = new pjs.Path.Circle({
      center : [sW-30,sH-12],
      radius : 6,
      fillColor : 'blue',
      data : 'sets'
