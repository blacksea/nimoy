...
module.exports = function Omni (opts) {
  ...
  function IO (d, e, n) {
    if (d.code === 1) { console.error(d); n(); return null } // check for err!
    if (d.cmd) {
    ...
    // pipe into pjs ui to draw tabs or update db
    if (d.key.split(':')[1] === opts.id) { // check for return message! 
  ...
  var db = r() // route to grab settings and such
  ...
  var grift = new function Grifter () {
    ...
    this.diff = function (newHash) {
      if (newHash instanceof Array) newHash = _.object(newHash)
      // do the diff
  ...
  var omni = new D({ 
    ...
    events : [ // FILTER / PASS UP TO CHAINS
      ['.lib li','dragstart',function dragModule (e) { // drop module onto doc.
        e.dataTransfer.setData('cmd','+'+e.target.innerHTML.split('</b>')[1])
      }],
      ['.settings input','keyup', function (e) {
        if (e.keyCode===13) cancel(e);
      }],
      [window,'contextmenu', function (e) { grift.put(drompii(e.target)) }],
  ...
  // drop image upload for textarea/input // GET RID OF DX! // use somekind of increment?
  ...
  function RightClickGrift (e) { // CONTEXT CLICK -- DROMPII
    cancel(e)
    blob = {}
    metaBlob = {}
    editor.erase()
  ...
  function GrifterEdit (e) { // UPDATE MODULE -- synce back with db interface
    var cmd 
  ...
  function updateDataAfterGrifting (pip) { // SAVE META AND NEW DATA AT ONCE!
  ...
  db.get('?$settings', function (e,d) { // call this when opening settings!
    if (!e) {
      blob = { cuid : 'settings', edit : { key : 'favicon' }, data : d }
      omni.draw(d)
      toggle('.settings')
    }
  })

  // GLOBAL/WINDOW/DOC STUFF
...
function buildUI () {
  ...
  var folds = {
    ...
    fold : function (sel) {
      // simplify -- it only takes one type of arg
  ...
  tabBar = new function Tabs () {
    ...
    this.new = function () { // new blank tab!
      pjs.view.draw()
    }
