...
module.exports = function Omni (opts) {
  ...
  var omni = new dombii({// data here instead of making a bunch of extra vars
    name : 'omni',
    id : opts.id,
    template : OMNI,
  ...
  function IO (d, e, n) {
    ...
    if (d.key && d.key.split(':')[1] === opts.id) { // check for return message! 
      if (d.key.split(':')[0] === '@edit') { // LOGGED IN!
        ...
        toggle('.login')
        _.defer(function () {gui.folds.to('cvs')})
  ...
  function buildUI () { // INCLUDE A STATUS UPDATE THING
    ...
    var panelUi = new pjs.Group([panelSlider,new pjs.Group([mask,tabs]),panelButtons])
    ...
    window.g = gui
    // if (!sessionStorage['edit']) gui.folds.to('login')
...
var Folds = function (panel) { // UI rigging
  ...
  this.to = function (celName) {
    ...
    pjs.view.draw()
    cutCvs(vH)
    if (celName==='prefs') toggle('.settings')
  }
}
var Tabs = function (tabsBg,tabsTxt,tabG) {
  ...
  function resizeTabs () {
    var max = Math.floor(((sW-48-(tabs.children.length*16))/tabs.children.length)/6)
    var tabOffset = 20
